...
[← Documentation](README.md) · [First-time setup](GETTING_STARTED.md)

The local Python coordinator connects Alpaca paper account state, completed IEX bars, two independent v783 Brian2 simulations, the selected neural readout, SQLite reconciliation, and the hosted desktop. Original, training-only and gated learned modes are described in [LEARNING-LAB.md](LEARNING-LAB.md). The synthetic demo remains separate.
...
Local `.env` contains `ALPACA_PAPER_API_KEY` and `ALPACA_PAPER_SECRET_KEY`. Local `.env.bridge` contains the desktop origin and its matching bridge key. The Vercel server holds a distinct owner key and Turso settings; Alpaca credentials stay local. A Sites bypass token is needed only for the legacy Sites host. See [VERCEL.md](VERCEL.md) for exact configuration.
...
- Hosted Turso/libSQL on Vercel (D1 on legacy Sites): latest telemetry, receipt timestamp, owner commands, research state and chart cache. Website storage is separate from the full local trading ledger.
- Worker synchronizes state about every 10–15 seconds via authenticated POST `/api/bridge` and receives the current command. Vercel uploads use gzip and recursive changed-field patches after the first full snapshot. A stale base triggers one full resync. Immediately before submission, a small authenticated GET checks controls without uploading history. Legacy Sites retains its original JSON exchange.
- The desktop polls `/api/backend?compact=1` every ten seconds while visible, stops while hidden, and refreshes on return. Versioned chunks transfer only changed fields; the browser reconstructs the complete snapshot. On Vercel, telemetry is public and mutations require the owner session plus a same-origin request. Platform identity headers do not grant Vercel owner access; worker and owner credentials are separate.
...
A stable symbol hash determines the ordinary sensory tour. The optional [Jev news scout](TYPESAFE.md) can alternate a news-priority candidate with a regular tour candidate, without advancing the regular cursor for priority picks. It runs in a background thread; failures and expired priorities fall back to the tour. The coordinator assigns available symbols to either of the two independent fly states. BUY/SELL intent is exclusively decoded from neural activity for the presented symbol; HOLD advances the tour. This is not a biological attention model or evidence of free will. The complete universe, its digest, cursor, last coverage per symbol, and context ID are recorded. The desktop exposes searchable membership and recent data gaps; detailed history remains local.
...
In Original mode, the decoder only receives measured BUY and SELL firing rates. Neither prices nor P&L enter it. The default is HOLD unless the higher pool reaches 20 Hz with at least an 8 Hz lead. There is no LLM in the decision path. Training-only and learned modes use the separately documented neural-feature memory; the underlying connectome remains fixed.
...
The desktop receives the latest 100 decisions/orders, 50 events, and at most 500 selected equity points covering the full recorded timespan. Older history is condensed with extrema and source gaps preserved; recent history gets its own plotting budget. Counts and date bounds distinguish plotted points from full records. The local ledger retains the full history. To export a full report, stop the worker, then run:
...
Response/ablation and checkpoint validation must match the current hashes of the dataset manifest, brain adapter, and encoder/decoder source. The Training Lab implements an initial held-out signal evaluation. Multi-seed profitability research, quote-relative slippage, corporate-action-aware portfolio backtests and stronger matched controls remain open work. Corporate-action valuation checks are implemented, but they do not constitute a complete portfolio accounting engine. The six-bar replay is a connectivity/integration pilot only.
...
[Corporate-action checks](CORPORATE-ACTIONS.md) run before new orders or trusted performance reporting. Affected account values remain available as raw evidence; profit metrics and contaminated learning observations are withheld. A known issue survives restarts, feed failures and disappearing positions. It requires audited reconciliation rather than a new baseline or fabricated fill.
