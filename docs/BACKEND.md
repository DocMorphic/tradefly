...
- The desktop polls `/api/backend` every five seconds. On Vercel, telemetry is public and mutations require the owner session plus a same-origin request. Platform identity headers do not grant Vercel owner access; worker and owner credentials are separate.
