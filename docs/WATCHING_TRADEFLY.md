...
The production configuration rotates through AAPL, MSFT, NVDA, AMZN, JPM, XOM, JNJ, WMT, CAT, NEE, DIS and PLD through Alpaca paper. Edit the ordered watchlist in Paper → Data & definitions while paused; 1–24 eligible fractional US stocks are supported. A newly completed five-minute regular-session bar triggers at most one neural decision for the next stock in a fixed rotation. With 12 symbols, each is revisited about once per market hour. One shared brain carries its state between stocks; ordering therefore affects the experiment. No price-ranking algorithm chooses which stock goes next. On a full US trading day, decisions can run after 09:35 through 15:55 New York time. The final 15:55–16:00 bar completes after regular trading closes, so the live worker does not place an order from it. Holidays and early closes follow Alpaca's exchange calendar. HOLD, execution constraints, pauses, or unavailable data reduce actual order frequency, potentially to zero.
