...
**Status: planned, not implemented.** No simulator, market-data connection, or order execution exists yet. No credentials are required to read this project or begin the offline work.
...
- Local Python service; SQLite event ledger and Parquet input data. A browser dashboard comes after the simulator and experiment are working.

These are planning choices, not implemented enforcement. See [the technical plan](docs/PLAN.md), [milestones](docs/ROADMAP.md), [user setup](docs/USER_SETUP.md), and [sources](docs/SOURCES.md).
