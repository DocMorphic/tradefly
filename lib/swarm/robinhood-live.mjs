...
export class RobinhoodLive {
  ...
  async rpc(method, params = [], timeout = 12000) {
    const allowed = new Set(['eth_call','eth_blockNumber','eth_getLogs','eth_getTransactionCount','eth_getCode']);
    if (!allowed.has(method)) throw new Error('Read-only RPC methods only');
  ...
  publicCache() {
    return {
      mode: this.cache.tokens.length ? (this.cache.error && this.cache.source.startsWith('Robinhood RPC') ? 'STALE' : 'LIVE') : 'DEGRADED',
  ...
  async search(query) {
    ...
    if (/^0x[a-fA-F0-9]{40}$/.test(q)) {
      const code = await this.rpc('eth_getCode', [q, 'latest']);
      if (!code || code === '0x') throw new Error('No contract found at that address');
  ...
  async liveHolders(address) {
    ...
    return {
      ...
      coverage: `sample of at most 160 observed addresses; blocks ${from}–${head}; up to 40 positive balances shown`,
