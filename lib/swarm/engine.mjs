// Adapted from semkazz1/FlySwarm @ 7237572b16bfdd02ec04b3d732f9badd27dbf8eb
// Copyright (c) 2026 Gyomei. MIT; see ./LICENSE. All cohort outputs are synthetic.
const HUNTERS = [
...
function mulberry32(seed) {
  ...
  random.setState = value => { seed = value | 0; };
  return random;
}
...
export class FlySwarmEngine {
  ...
  serialize() {
    return { seed:this.seed, rng:this.random.getState(), tickNo:this.tickNo,
      transfers:this.transfers, signals:this.signals, positions:this.positions,
      activeToken:this.activeToken, focusToken:this.focusToken, config:this.config, network:this.network };
  }
  static restore(record) {
    const engine = new FlySwarmEngine(record.seed);
    for (const key of ['tickNo','transfers','signals','positions','activeToken','focusToken','config','network']) engine[key]=record[key];
    engine.random.setState(record.rng);
    return engine;
  }
  snapshot() {
    ...
    return {
      meta: {
        mode: 'SYNTHETIC', seed: this.seed, tick: this.tickNo,
        disclaimer: 'Synthetic wallet histories, funding routes, signals and price paths. No biological brain, signer, broker order or real profitability measurement.'
      },
      pulse: {
